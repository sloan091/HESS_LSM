...
if isequal(Flag.Tflag,1) && isequal(Flag.Cflag,1)
    ...
    [A_n_sl,A_sl,R_d_sl,ContA_sl] = FvCB_C3wTemp(V_max_sl,J_max_sl,...
        Plant.Sig_psii,Q_sl,Plant.Theta_psii,BC.P_atm,IS_DVs.c_i_sl,...
        OS_DVs.T_l_sl,Const.R_g);
    [A_n_sh,A_sh,R_d_sh,ContA_sh] = FvCB_C3wTemp(V_max_sh,J_max_sh,...
        Plant.Sig_psii,Q_sh,Plant.Theta_psii,BC.P_atm,IS_DVs.c_i_sh,...
        OS_DVs.T_l_sh,Const.R_g);
elseif isequal(Flag.Tflag,1) && isequal(Flag.Cflag,2)
    % C4 photosynthesis with temperature dependence
    [A_n_sl,A_sl,R_d_sl,ContA_sl] = Collatz_C4wTemp(V_max_sl,Q_sl,BC.P_atm,...
        IS_DVs.c_i_sl,OS_DVs.T_l_sl);
    [A_n_sh,A_sh,R_d_sh,ContA_sh] = Collatz_C4wTemp(V_max_sh,Q_sh,BC.P_atm,...
        IS_DVs.c_i_sh,OS_DVs.T_l_sh);
elseif isequal(Flag.Tflag,0) && isequal(Flag.Cflag,2)
    % C4 photosynthesis without temperature dependence
    [A_n_sl,A_sl,R_d_sl,ContA_sl] = Collatz_C4(V_max_sl,Q_sl,...
        BC.P_atm,IS_DVs.c_i_sl);
    [A_n_sh,A_sh,R_d_sh,ContA_sh] = Collatz_C4(V_max_sh,Q_sh,...
        BC.P_atm,IS_DVs.c_i_sh);
else
