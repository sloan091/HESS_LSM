% =========================================================================
% Name   : Inner_Solver_PHM.m
% Author : Brandon Sloan
% Date   : 5/26/21
%
% DESCRIPTION
% This function is the inner solver of the LSM with a PHM downregulation
...
% values given by the PHM. For reference, this code covers from the fourth
...
function IS_Results = Inner_Solver_PHM(OS_DVs,Const,Rad,BC,...
    Plant,Soil,Flag)
...
% CO2 conductance from inside leaf to canopy airspace [moles air/m^2 LA/s]
...
g_sl_phm = Plant.ns*MolFlux2mps(Rad.LAI_sl*(IS_Results.g_bvl...
    *OS_DVs.g_s_sl)/(IS_Results.g_bvl +...
    OS_DVs.g_s_sl),BC.P_atm,OS_DVs.T_l_sl); 
g_sh_phm = Plant.ns*MolFlux2mps(Rad.LAI_sh*(IS_Results.g_bvl...
...
e_ca = (IS_Results.g_av*BC.e_a + g_sl_phm*IS_Results.e_i_sl + ...
    g_sh_phm*IS_Results.e_i_sh + IS_Results.g_av_g*IS_Results.e_g)...
    /(IS_Results.g_av + g_sl_phm + g_sh_phm + IS_Results.g_av_g);
...
% Sunlit leaf to canopy airspace [W/m^2]
IS_Results.E_sl = Const.rho_a*g_sl_phm*(Const.M_w/Const.M_a/BC.P_atm)*...
    (IS_Results.e_i_sl - e_ca)*Const.L_vap;
% Shaded leaf to canopy airspace [W/m^2]
IS_Results.E_sh = Const.rho_a*g_sh_phm*(Const.M_w/Const.M_a/BC.P_atm)*...
...
IS_Results.g_sl_c = g_sl_phm;
IS_Results.g_sh_c = g_sh_phm;
