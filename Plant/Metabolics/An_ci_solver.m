...
fun_sl = @(c_i_sl) Solver_sl(c_i_sl,g_sl_CO2);
c_i_sl = fzero(fun_sl,c_int,options);
[Resid_sl,A_n_sl] = Solver_sl(c_i_sl,g_sl_CO2);

% Setup, run and store shaded solver
fun_sh = @(c_i_sh) Solver_sh(c_i_sh,g_sh_CO2);
c_i_sh = fzero(fun_sh,c_int,options);
[Resid_sh,A_n_sh] = Solver_sh(c_i_sh,g_sh_CO2);
