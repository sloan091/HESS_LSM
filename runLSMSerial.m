...
% the LSM runs serially which takes longer than the parallel version but is
% easier to debug and run short simulations.
%
% INPUTS
...
LSM_Results_WW   = Create_Outputs(size(FluxData,1));
LSM_Results_PHM  = Create_Outputs(size(FluxData,1));
LSM_Results_Beta = Create_Outputs(size(FluxData,1));
...
for step = 1:size(FluxData,1)
    ...
    try
        ...
        LSM_step_WW = Store_Outputs(Rad,StepResults,exitflag);
    ...
    if isequal(Flag.PHMflag,1)
        ...
        if isequal(WWerror,0)
            ...
            try
                ...
                LSM_step_PHM = Store_Outputs(Rad,StepResults,exitflag);
    ...
    if isequal(Flag.Betaflag,1) || isequal(Flag.Betaflag,2)
        ...
        if isequal(WWerror,0)
            ...
            try
                ...
                LSM_step_Beta = Store_Outputs(Rad,StepResults,exitflag);
